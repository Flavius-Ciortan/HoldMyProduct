 ...
 */
class Hold_This_Product_Analytics {
    ...
    public function enqueue_analytics_scripts( $hook ) {
        ...
        if ( $hook === 'holdthisproduct_page_holdthisproduct-analytics' || $page === 'holdthisproduct-analytics' ) {
            wp_enqueue_style(
                ...
                HOLD_THIS_PRODUCT_PLUGIN_URL . 'assets/css/admin-style.css',
                array(),
    ...
    public function analytics_page() {
        ...
        <div class="wrap">
            ...
            <div class="hold-this-product-stats-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; margin: 20px 0;">
                <div class="hold-this-product-stat-card" style="background: #fff; padding: 20px; border: 1px solid #ddd; border-radius: 8px;">
    ...
    private function get_reservation_stats() {
        global $wpdb;
		$rows = $wpdb->get_results( "SELECT pm.meta_value AS reservation_status, COUNT(*) AS reservation_count FROM {$wpdb->posts} p INNER JOIN {$wpdb->postmeta} pm ON p.ID = pm.post_id AND pm.meta_key = '_hold_this_product_status' WHERE p.post_type = 'holdthisproduct_res' AND p.post_status = 'publish' GROUP BY pm.meta_value", OBJECT_K ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- No external values.
    ...
    private function display_recent_reservations() {
        $reservations = get_posts( array(
            ...
            'posts_per_page' => 20,
            'meta_key' => '_hold_this_product_status',
            'orderby' => 'date',
            'order' => 'DESC'
        ) );
        ...
        foreach ( $reservations as $reservation ) {
            $product_id = get_post_meta( $reservation->ID, '_hold_this_product_product_id', true );
            ...
            $expires_ts = get_post_meta( $reservation->ID, '_hold_this_product_expires_at', true );
